...
##### Step 3. 
The next step is to combine the base topic models using an ensemble approach, to produce a final ensemble model. Note that we specify all of the factor files from the base topic models to combine, along with the number of overall ensemble topics (here it is again *k=4*). The model will be written to the directory 'models/ensemble'.
...
We can display the top 10 terms in the topic descriptors for the final ensemble results:
	...
	python display-topics.py models/ensemble/ranks_ensemble_k04.pkl 
